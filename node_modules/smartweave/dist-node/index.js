...
var promisesTho = require('promises-tho');
...
async function createContract(arweave, wallet, contractSrc, initState, minFee) {
  let srcTx = await arweave.createTransaction({
    data: contractSrc
  }, wallet);
  ...
  if (response.status == 200 || response.status == 208) return createContractFromTx(arweave, wallet, srcTx.id, initState, minFee);else throw new Error(`Unable to write Contract Source.`);
}
/**
...
async function createContractFromTx(arweave, wallet, srcTxId, state, minFee) {
  ...
  if (response.status == 200 || response.status == 208) return contractTX.id;else throw new Error(`Unable to write Contract Initial State`);
}
...
function unpackTags(tx) {
  ...
  for (let i = 0; i < tags.length; i++) {
    try {
      const name = tags[i].get('name', {
        decode: true,
        string: true
      });
      const value = tags[i].get('value', {
        decode: true,
        string: true
      });
...
async function readContract(arweave, contractId, height = Number.POSITIVE_INFINITY) {
  ...
  const getTxInfoFn = promisesTho.retryWithBackoff({
    tries: 3,
    startMs: 1000
  }, id => getFullTxInfo(arweave, id));
  const batcher = promisesTho.batch({
    batchDelayMs: 50,
    batchSize: 3
  }, promisesTho.softFailWith(undefined, getTxInfoFn));
  console.log(`Query returned ${transactions.length} interactions`);
  let unconfirmed = await batcher(transactions);
  console.log(`Recieved info for ${unconfirmed.length} transactions`); // Filter out txs that are not confirmed yet, not found, 
  // or are below the height we are replaying to.

  let txInfos = unconfirmed.filter(x => x && x.info.confirmed && x.info.confirmed.block_height <= height);
...
async function getFullTxInfo(arweave, id) {
  const [tx, info] = await Promise.all([arweave.transactions.get(id).catch(e => {
...
async function loadContract(arweave, contractID) {
  try {
    ...
    const state = contractTX.get('data', {
      decode: true,
      string: true
    ...
    const {
      handler,
      swGlobal
    } = createContractExecutionEnvironment(arweave, contractSrc);
    return {
      id: contractID,
...
function createContractExecutionEnvironment(arweave, contractSrc) {
  ...
  const getContractFunction = new Function('swGlobal', returningSrc); //console.log(returningSrc);
