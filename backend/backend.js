  ...
  return Promise(async (resolve, reject) => {
    try {
      ...
      if (existingWallet == 'Y') {
        ...
        var wallet_private_key = JSON.parse(
      ...
      const wallet_public_key = await arweave.wallets.jwkToAddress(
        wallet_private_key
      );
      const encrypted_wallet_private_key = await ArDriveCrypto.encryptText(
        JSON.stringify(wallet_private_key),
        loginPasswordResponse.password
      );
      const encrypted_dataProtectionKey = await ArDriveCrypto.encryptText(
      ...
      resolve({
        ...
        jwk: JSON.stringify(wallet_private_key),
        wallet_public_key,
        owner,
        sync_folder_path: syncFolderPath,
      });
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      const response = await fetch(
        `https://us-east.perma.online/price/${bytes}/${target}`
      );
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      await asyncForEach(allFiles, async (file) => {
        ...
        const file_name = path.basename(fullpath);
        ...
        if (extension != '.enc' && stats.size != 0) {
          ...
          let ardrive_path = fullpath.replace(user.sync_folder_path, '');
          ...
          if (matchingArDriveID) {
            ...
            isCompleted = await arDriveFiles.getByFileName_fromCompleted(
              file_name
            );
        ...
        if (readyToUpload == 'Y') {
          ...
          await asyncForEach(filesToUpload, async (fileToUpload) => {
            if (fileToUpload.file_size == '0') {
              ...
              transaction = await uploadArDriveFile(
                user,
                fileToUpload.file_path,
...
async function uploadArDriveFile(
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const file_to_upload = fs.readFileSync(file_path);
      const file_name = path.basename(file_path.replace('.enc', ''));
      const transaction = await arweave.createTransaction(
        { data: arweave.utils.concatBuffers([file_to_upload]) },
        JSON.parse(user.jwk)
      );
        ...
        try {
          const transaction = await arweave.transactions
            ...
            .then(async (transaction) => {
              const isCompleted = await arDriveFiles.getByTx_fromCompleted(
                txid
              );
              if (isCompleted) {
...
async function checkIfQueued(arDrive_id) {
  ...
  const row = await arDriveFiles.getByArDriveId_fromQueue(arDrive_id);
  console.log(row);
}
...
async function main() {
  try {
    ...
    if (profile == '') {
      ...
      var user = await unlockArDriveProfile(
    ...
    while (true) {
      ...
      const getfiles = await getMyArDriveFiles(user);
      const queuedfiles = await queueNewFiles(user, user.sync_folder_path);
      const checkedstatus = await checkUploadStatus(user);
      const uploadedfiles = await uploadArDriveFiles(user);
      const downloadfiles = await downloadMyArDriveFiles(user);
